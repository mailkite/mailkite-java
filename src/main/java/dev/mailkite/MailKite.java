...
public class MailKite {
  ...
  /** {@link #pageQuery(Long, Integer)} plus an optional {@code search} substring filter. */
  private static String pageQuery(Long before, Integer limit, String search) {
    String qs = pageQuery(before, limit);
    if (search != null) {
      qs += (qs.isEmpty() ? "?" : "&")
          + "search="
  ...
  public Object listMessages() {
    return listMessages(null, null, null);
  }
  ...
  public Object listMessages(Long before, Integer limit) {
    return listMessages(before, limit, null);
  }

  /**
   * List messages with optional {@code before} cursor, {@code limit} page size, and a
   * {@code search} substring filter (matches sender, recipient, or subject).
   */
  public Object listMessages(Long before, Integer limit, String search) {
    return request("GET", "/api/messages" + pageQuery(before, limit, search), null);
  }
